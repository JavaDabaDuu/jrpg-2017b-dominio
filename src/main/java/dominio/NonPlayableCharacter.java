...
public class NonPlayableCharacter extends MadreDeTodo implements Peleable {
    ...
    */
    public NonPlayableCharacter(final String nombre, final int nivel,
        ...
        this.aumentarFuerza(
                MODIFICADOR_BASE_FUERZA * (dificultad + 1) + (nivel - 1)
            * MULTIPLICADOR_FUERZA * (dificultad + 1));
        this.salud = MODIFICADOR_BASE_SALUD
                  * (dificultad + 1) + (nivel - 1) * MULTIPLICADOR_SALUD
                  * (dificultad + 1);
        this.aumentarDefensa(
            MODIFICADOR_BASE_DEFENSA * (dificultad + 1) + (nivel - 1)
            * MULTIPLICADOR_DEFENSA * (dificultad + 1));
        }
    ...
    */
    public final void setSalud(final int sal) {
        this.salud = sal;
        }
    ...
    */
    ...
    public final int atacar(final Peleable atacado) {
        if (this.getRandom().nextDouble() <= NUMERO_PARA_ATACAR) {
            ...
            * MULTIPLICADOR_FUERZA_GOLPE_CRITICO));
            } else {
