...
public class Asesino extends Casta {
    ...
    * sobre la probabilidad de evitar un daño.
    */
    ...
    */
    ...
    public final boolean habilidad2(final Personaje caster,
        ...
        if (caster.getEnergia() >= ENERGIAMINIMA) {
            caster.reducirEnergia(ENERGIAMINIMA);
            if (this.getProbabilidadEvitarDanio()
                + AUMENTAR_PROBABILIDAD_EVITAR < PROBABILIDAD_EVITAR_DANIO) {
                this.aumentarEvitarDanio(AUMENTAR_PROBABILIDAD_EVITAR);
                } else {
                    this.setProbabilidadEvitarDanio(PROBABILIDAD_EVITAR_DANIO);
                    }
            return CASTEO_EXITOSO;
            }
        return CASTEO_FALLIDO;
